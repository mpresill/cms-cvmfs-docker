...
parser.add_argument('--decorate-cl', default=None)
parser.add_argument('--legend-off', default=0.0, type=float, help='legend x-offset')
parser.add_argument('--title-right', default='35.9-137 fb^{-1} (13 TeV)')
...
for i, yval in enumerate(yvals):
    ...
    if args.decorate_cl:
        ...
        latex.SetTextAlign(12)
        latex.SetTextColor(14)
        latex.DrawLatex(axishist.GetXaxis().GetXmax() * 0.80, yval+0.3, '%.0f%% CL' % (ROOT.Math.chisquared_cdf(yval, 1) * 100.))
